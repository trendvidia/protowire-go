...
// can skip its runtime typecheck because the codec template was built
// from the same descriptor and already knows fd's [protoreflect.Kind].
// That avoids the v.Interface() boxing alloc (~16 B per scalar set on
// values outside Go's small-int pool) — roughly half the per-Unmarshal
// allocations on dynamicpb-backed messages.
//
// For generated [proto.Message] types the fast path doesn't apply, so
